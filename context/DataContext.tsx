...
export const DataProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const addService = (service: Omit<Service, "id">) =>
    apiRequest("/services", "POST", service);
  const updateService = (service: Service) =>
    apiRequest(`/services/${service.id}`, "PUT", service);
  const deleteService = (id: string) => apiRequest(`/services/${id}`, "DELETE");

  const addAppointment = (appointment: Omit<Appointment, "id" | "createdAt">) =>
    apiRequest("/appointments", "POST", appointment);
  const deleteAppointment = (id: string) =>
    apiRequest(`/appointments/${id}`, "DELETE");

  const updateHomeContent = (content: HomePageContent) =>
    apiRequest("/content", "PUT", content);
