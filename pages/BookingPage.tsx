...
const BookingPage: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-dark-1 flex items-center justify-center text-brand-primary text-2xl">
        Carregando...
      </div>
    );
  }
                ...
                <div
                  ...
                  className={`p-4 rounded-lg cursor-pointer transition-all border-2 ${
                    selectedService?.id === service.id
                      ...
                      : "bg-dark-3 border-transparent hover:border-brand-primary/50"
                  }`}
                >
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-48 overflow-y-auto">
                  {availableTimes.length > 0 ? (
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg transition-colors ${
                          selectedTime === time
                            ...
                            : "bg-dark-3 hover:bg-brand-primary/50"
                        }`}
                      >
                        {time}
                      </button>
                    ...
                    <p className="col-span-full text-light-3">
        ...
        return (
          <div>
            ...
            <div className="bg-dark-3 p-6 rounded-lg space-y-4">
              <div>
                <strong className="text-brand-secondary">Nome:</strong>{" "}
                {clientName}
              </div>
              <div>
                <strong className="text-brand-secondary">Telefone:</strong>{" "}
                {clientPhone}
              </div>
              {clientInstagram && (
                <div>
                  <strong className="text-brand-secondary">Instagram:</strong>{" "}
                  {clientInstagram}
                </div>
              )}
              <div>
                <strong className="text-brand-secondary">Serviço:</strong>{" "}
                {selectedService?.name}
              </div>
              <div>
                <strong className="text-brand-secondary">Data:</strong>{" "}
                {selectedDate &&
              ...
              <div>
                <strong className="text-brand-secondary">Hora:</strong>{" "}
                {selectedTime}
              </div>
              <div className="text-xl font-bold pt-4 border-t border-dark-2">
                <strong className="text-brand-secondary">Total:</strong> R${" "}
        ...
        return (
          <div className="text-center">
            ...
            <p className="text-light-2 mb-8">
              Seu horário foi reservado com sucesso. Obrigado!
            </p>
            <Link
              to="/"
  ...
  return (
    <div className="min-h-screen bg-dark-1 flex flex-col items-center justify-center p-4">
          ...
          <>
            <div className="mb-8">
              <div className="flex justify-between text-sm font-semibold text-light-3 mb-2">
                <span className={step >= 1 ? "text-brand-primary" : ""}>
                  Pessoal
                </span>
                <span className={step >= 2 ? "text-brand-primary" : ""}>
                  Serviço
                </span>
                <span className={step >= 3 ? "text-brand-primary" : ""}>
                  Data/Hora
                </span>
                <span className={step >= 4 ? "text-brand-primary" : ""}>
                  Confirmar
                </span>
              </div>
            ...
            <div className="flex justify-between mt-8">
              {step > 1 ? (
                <button
                  ...
                  className="bg-dark-3 text-light-1 font-bold py-2 px-6 rounded-lg hover:bg-dark-3/80 transition-colors"
                >
                  Voltar
                </button>
              ) : (
              ...
              {step < 4 ? (
                <button
                  ...
                  className="bg-brand-primary text-dark-1 font-bold py-2 px-6 rounded-lg hover:bg-brand-secondary transition-colors"
                >
                  Próximo
                </button>
              ) : (
                <button
                  ...
                  className="bg-green-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-500"
                >
      ...
      {step < 5 && (
        <Link to="/" className="mt-4 text-brand-primary hover:underline">
